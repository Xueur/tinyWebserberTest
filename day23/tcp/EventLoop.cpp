...
void EventLoop::RunAt(TimeStamp timestamp, std::function<void()> const & cb) {
    timer_queue->addTimer(timestamp, std::move(cb), 0.0);
}

void EventLoop::RunAfter(double wait_time, std::function < void()>const & cb) {
    timer_queue->addTimer(TimeStamp::addTime(TimeStamp::now(), wait_time), std::move(cb), 0.0);
}

void EventLoop::RunEvery(double interval, std::function<void()> const & cb) {
    timer_queue->addTimer(TimeStamp::addTime(TimeStamp::now(), interval), std::move(cb), interval);
}